...
class TBCDByteField(StrFixedLenField):
    ...
    def i2m(self, pkt, val):
        ...
        for i in range(0, len(val), 2):
            tmp = val[i:i + 2]
            if len(tmp) == 2:
                ret += struct_byte.pack(int(tmp[1] + tmp[0], 16))
            else:
                ret += struct_byte.pack(int("F" + tmp[0], 16))
        return ret
...
class APNStrLenField(StrLenField):
    ...
    #     r = b""
    #     for x in s.split('.'):
    #         r += struct_byte.pack(len(x)) + x.encode()
    #     return r

    def i2m(self, pkt, s):
        s = str(s).split("'")[1]
        r = b"".join(struct_byte.pack(len(x)) + x.encode() for x in s.split("."))
        return r
