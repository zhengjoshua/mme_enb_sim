...
class IE_MEI(gtp.IE_Base):
    ...
    fields_desc = [ByteEnumField("ietype", 75, IEType),
                   ...
                   BitField("CR_flag", 0, 4),
                   BitField("instance", 0, 4),
                   gtp.TBCDByteField("MEI", "123456789012345",
...
class IE_IMSI(gtp.IE_Base):
    ...
    # def post_build(self, p, pay):
    #     print("post build p", p)
    #     print("post build pay", pay)
    #     print("post build length", self.length)
    #     p += pay
    #     if self.length == 0:
    #         l = len(p)
    #         p = p[:2] + struct.pack("!H", l) + p[4:]
...
class IE_MSISDN(gtp.IE_Base):
    ...
    fields_desc = [ByteEnumField("ietype", 76, IEType),
                   ...
                   BitField("CR_flag", 0, 4),
                   BitField("instance", 0, 4),
                   gtp.TBCDByteField("MSISDN", "33123456789",
                                        length_from=lambda x: x.length)]
...
class IE_NodeFeatures(gtp.IE_Base):
    name = "IE Node Features"
